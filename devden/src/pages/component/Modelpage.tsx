...
import { Box, Dialog, DialogActions, Button, Typography,Grid,CardActionArea,Card,CardContent } from '@mui/material';
...
const Modelpage = ({ open, handleClose }) => {
  ...
  return (
      ...
      <Box sx={{ flexGrow: 1, padding: 3, backgroundColor: 'rgb(33, 44, 74)', color: 'white' }}>
        ...
        <Typography variant="h6" gutterBottom>
          ...
          {[
            ...
            <Grid item xs={6} sm={6} md={4} key={index} display={'center'}>
              <Card sx={{ backgroundColor: 'rgb(11, 22, 44)', color: 'white', '&:hover': { backgroundColor: 'rgb(22, 33, 55)' } }}>
        ...
        <Box sx={{ marginTop: '25px' }}>
          ...
          <Grid container spacing={3}>
            {[
              { title: 'JavaScript DOM Exercises', description: 'Create interactive and dynamic websites.', path: '/dom-exercises' },
              { title: 'JavaScript Practice', description: 'Challenges to practice your skills.', path: '/practice' },
            ].map((item, index) => (
              <Grid item xs={6} sm={6} key={index} display={'center'}>
                <Card sx={{ backgroundColor: 'rgb(11, 22, 44)', color: 'white', '&:hover': { backgroundColor: 'rgb(22, 33, 55)' } }}>
                  <CardActionArea component={Link} href={item.path} sx={{ textAlign: 'left', display:'block', padding: '10px' }}>
        ...
        <DialogActions>
          <Button onClick={handleClose} style={{ backgroundColor: 'white' }} color="error">
            Close
          </Button>
        </DialogActions>
      </Box>
